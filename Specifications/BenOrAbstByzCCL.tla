---- MODULE BenOrAbstByzCCL ----
(* This specification is a refinement of BenOrAbstByzCC that contains a node action
    for tossing the common coin. Now instead of the coin being tossed by the system, some
    node tosses the coin.
   The refinement mapping RefAbstCC is also included in the specification and can be checked with TLC.
   The specification contains fewer distinct states compared to BenOrAbstByzCC but as the number
    of nodes increase the amount of total states becomes greater than BenOrAbstByzCC.
*)
...
\* RefinementProperty == RefAbstCC!Spec \* Can be uncommented here and in the BenOrByzCC.cfg file to check refinement.
