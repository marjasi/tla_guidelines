---- MODULE BenOrAbstByzCCLH ----
(* This is a refinement of BenOrMsgsByzCCL, BenOrAbstByzCCL, and BenOrMsgsByzCCLH with additional refinement history variables.
   Several modifications were required for TLC to successfully verify the refinement mappings RefMsgs:
     ...
     - Modified the Init formula to also set the estimateHistory values based on the initial estimates similarly to BenOrMsgsByzCCL.
     - Added actions for checking whether certain messages are present in the msgsHistory variable. Added corresponding checks for
        ...
        state space in a way to make the mapping RefAbstCCL incorrect, and is required for TLC to verify the mapping RefMsgs.
   ...
   The refinement mappings RefMsgsH and RefAbstCCL are also provided and can be checked with TLC if put inside the RefinementProperty
