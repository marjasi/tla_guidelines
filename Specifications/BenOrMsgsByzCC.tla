...
(* This specification is mostly the same as BenOrMsgsByz with some additional changes:
   ...
   The common coin is used to model successful termination of the BenOr Byzantine consensus
    algorithm with all possible inputs given a number of nodes N.
...
(* Set the Common Coin value for rounds that have been reached by nodes.
    ...
    /\ \E round \in CCRounds:
        ...
        /\ \/ \E node \in NODES \ FAULTY_NODES: \* If at least 1 node already estimated some value in phase 2b.
                    ...
                    /\ \/ CheckPhase2a(node) /\ CheckPhase2b(node)
                       \/ ~CheckPhase2a(node) /\ CheckPhase2b(node)
                    /\ commonCoin' = [commonCoin EXCEPT ![round] = estimates[node]]
...
Spec ==
    ...
    /\ [][Next]_vars 
    /\ \A node \in NODES:
        WF_vars(Phase1(node) \/ Phase2(node))
