---- MODULE BenOrMsgsByzCCH ----
(* This is a refinement of BenOrMsgsByzCC, BenOrAbstByzCC, and BenOrAbstByzCCH with additional refinement history variables.
   Several modifications were required for TLC to successfully verify the refinement mapping RefAbst:
     - Added the estimateHistory and proposalHistory variables that correspond to the estimatesAtRound and proposalsAtRound
        variables from BenOrAbstByzCC. All other variables are the same between both specifications and didn't require any
        additional changes.
     ...
     - Defined a new action SetNextRoundEstimate that sets the estimateHistory values for the next round. This behavior is the same
        as the SetNextRoundEstimate action in BenOrAbstByzCC.
     - Added an additional condition for the common coin action that requires for the estimateHistory variable to be set when tossing
        ...
        the state space in a way that would make the refinement mapping RefMsgsCC incorrect.
   The refinement mappings RefMsgsCC and RefAbstH are also provided and can be checked with TLC if put inside the RefinementProperty
...
TossCommonCoin ==
    /\ \E round \in CCRounds:
        ...
        /\ \/ \E node \in NODES \ FAULTY_NODES: \* If at least 1 node already estimated some value in phase 2b.
                 /\ estimateHistory[round][node] # "-1"
                 /\ WaitForProposals(node)
                 /\ \/ CheckPhase2a(node) /\ CheckPhase2b(node)
                    \/ ~CheckPhase2a(node) /\ CheckPhase2b(node)
                 /\ commonCoin' = [commonCoin EXCEPT ![round] = estimateHistory[round][node]]
