...
(* This is a refinement of BenOrMsgsByzCCL, BenOrAbstByzCCL, and BenOrAbstByzCCLH with additional refinement history variables.
   Several modifications were required for TLC to successfully verify the refinement mapping RefAbst:
     - Added the estimateHistory and proposalHistory variables that correspond to the estimatesAtRound and proposalsAtRound
        variables from BenOrAbstByzCCL. All other variables are the same between both specifications and didn't require any
        additional changes.
     ...
     - Defined a new action SetNextRoundEstimate that sets the estimateHistory values for the next round. This behavior is the same
        as the SetNextRoundEstimate action in BenOrAbstByzCCL.
     - Added an additional condition for the common coin action that requires for the estimateHistory variable to be set when tossing
        ...
        the state space in a way that would make the refinement mapping RefMsgsCCL incorrect.
   The refinement mappings RefMsgsCCL and RefAbstH are also provided and can be checked with TLC if put inside the RefinementProperty
