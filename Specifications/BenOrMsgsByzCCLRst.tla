---- MODULE BenOrMsgsByzCCLRst ----
(* This specification is a refinement of BenOrMsgsByzCCL that allows for only one non-faulty node CCNode
    ...
    to progress.
   Other BenOrMsgsByz common coin specifications have their own disadvantages:
    - In BenOrMsgsByzCC, the coin is tossed by the system, no one specific node, yet the state space is a lot greater due to the
       coin toss being a system action.
    - In BenOrMsgsByzCCL, any node can toss the coin, but this leads to a growth of the total number of states as the number of
       nodes increases.
   The refinement mapping RefMsgsCCL is also included in the specification and can be checked with TLC.
   The specification contains fewer distinct and total states compared to BenOrMsgsByzCCL and as the number
...
\* RefinementProperty == RefMsgsCCL!Spec \* Can be uncommented here and in the BenOrByzCC.cfg file to check refinement.
